...
//            display dropdown for societe
        echo "<tr class='tab_bg_1'>";
        echo "<td style='font-weight:bold;text-align:center;font-size:1.1em;background-color:#f1f1f1;'>" . __('Societe') . "</td>";
        echo "<td>";
        echo "<div class='select2-container'><select class='select2-choice' name='societe'><option value=''>None</option>";
        $query = "SELECT * FROM custom_societe";
        $result = $DB->query($query);
        while ($row = $DB->fetch_array($result)) {
            echo "<option value='" . $row['rowid'] . "'>" . $row['nom'] . " - " . $row['name_alias'] . "</option>";
        };
        echo "</select></div>";
        echo "</td>";
        echo "</tr>";
